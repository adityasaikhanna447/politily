...
function buildDocumentXml(story: StoredStory) {
  ...
  const sections: Array<[string, string[]]> = [
    ...
    ["Questions this video will answer", brief?.masterScriptQuestions?.length ? brief.masterScriptQuestions : ["No master script questions generated yet."]],
    ["Master script outline", brief?.masterScriptOutline?.length ? brief.masterScriptOutline : ["No master script outline generated yet."]],
    ["STEPPS strategy", brief?.steppsStrategy?.length ? brief.steppsStrategy : ["No STEPPS strategy generated yet."]],
    ...
    ["Compiled Roman Hindi master script", [brief?.videoScript || story.scriptText || "Generate a brief for the Roman Hindi master script."]],
    ["Script conclusion", [brief?.scriptConclusion || "Generate a brief for the final conclusion."]],
